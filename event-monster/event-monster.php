 ...
 **/
function pippin_get_image_urls( $image_url ) {
    // Get the attachment ID based on the URL
    ...
    if ( $attachment_id ) {
        ...
        if ( $image_sizes && isset( $image_sizes['sizes'] ) ) {
            // Construct image URLs for thumbnail, medium, and large sizes
            foreach ( $image_sizes['sizes'] as $size => $data ) {
                if ( in_array( $size, array( 'thumbnail', 'medium', 'large' ) ) ) {
                    $image_urls[$size] = wp_get_attachment_image_url( $attachment_id, $size );
                }
            }
        }

        // Get the full-size image URL
        $image_urls['full'] = wp_get_attachment_image_url( $attachment_id, 'full' );
    }
...
if ( ! class_exists( 'Awl_Event_Monster' ) ) {
	class Awl_Event_Monster {
		...
		public function Event_Monster() {
			$labels = array(
				'name'               => _x( 'Event Monster', 'event-monster' ),
				'singular_name'      => _x( 'Event Monster', 'event-monster' ),
			...
			if (current_user_can('manage_options')) {
				if ( isset( $_POST['em_save_nonce'] ) ) {
					if ( isset( $_POST['em_save_nonce'] ) && wp_verify_nonce( $_POST['em_save_nonce'], 'em_save_settings' ) ) {
						...
						update_post_meta( $post_id, $awl_event_monster_shortcode_setting, $event_monster_settings );
					}
				}
			}
