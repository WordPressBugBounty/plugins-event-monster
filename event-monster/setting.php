...
if ( isset( $_POST['em_setting_action'] ) ) {
	...
	if ( wp_verify_nonce( $em_save_nonce_value, 'em_save_nonce' ) ) {
		...
		update_option( 'awl_em_common_settings', $emcommonsettings  );
	}
} // end of save if
...
if ( isset( $_POST['action'] ) ) {
	...
	if ( $action == 'event-cancellation' ) {
		...
		$event_monster_output_settings = get_post_meta( $em_id_cancel_last, 'awl_em_settings_' . $em_id_cancel_last, true );
