...
function em_shortcode( $atts ) {
    ...
    if ( ! empty( $post_id ) ) {
        $event_monster_output_settings = get_post_meta( $post_id, 'awl_em_settings_' . $post_id, true );

        // Check if $event_monster_output_settings is not empty
        if ( ! empty( $event_monster_output_settings ) ) {
            $em_id                         = $post_id;
            ...
            // common settings
            $em_common_settings        = get_option('awl_em_common_settings');
            $em_main_font_size         = isset($em_common_settings['em_main_font_size']) ? $em_common_settings['em_main_font_size'] : "25";
            $em_sub_font_size          = isset($em_common_settings['em_sub_font_size']) ? $em_common_settings['em_sub_font_size'] : "22";
            $em_nrml_font_size         = isset($em_common_settings['em_nrml_font_size']) ? $em_common_settings['em_nrml_font_size'] : "18";
            $em_form_first_name_field  = isset($em_common_settings['em_form_first_name_field']) ? $em_common_settings['em_form_first_name_field'] : "First Name";
            $em_form_last_name_field   = isset($em_common_settings['em_form_last_name_field']) ? $em_common_settings['em_form_last_name_field'] : "Last Name";
            $em_form_email_field       = isset($em_common_settings['em_form_email_field']) ? $em_common_settings['em_form_email_field'] : "Email";
            $em_form_phone_field       = isset($em_common_settings['em_form_phone_field']) ? $em_common_settings['em_form_phone_field'] : "Phone";
            $em_form_heading_one       = isset($em_common_settings['em_form_heading_one']) ? $em_common_settings['em_form_heading_one'] : "Book Ticket Now";
            $em_form_sub_heading       = isset($em_common_settings['em_form_sub_heading']) ? $em_common_settings['em_form_sub_heading'] : "Fill in the form below to get instant access:";
            $em_form_sucsess_msg_field = isset($em_common_settings['em_form_sucsess_msg_field']) ? $em_common_settings['em_form_sucsess_msg_field'] : "Your Booking has been completed!!";
            $em_booking_full_msg       = isset($em_common_settings['em_booking_full_msg']) ? $em_common_settings['em_booking_full_msg'] : "All Tickets are Booked!!";

            $allslides = array(
                'p'         => $em_id,
                'post_type' => 'awl_event_monster',
                'orderby'   => 'ASC',
            );
            ...
            while ($loop->have_posts()) : $loop->the_post();
                ...
                // include event monster output file
        ...
        // sorting event by upcoming dates
        $all_event_dates = array();
        if ($loop->have_posts()) {
            while ($loop->have_posts()) : $loop->the_post();
                $event_id = get_the_ID();
                $event_monster_output_settings = get_post_meta(get_the_ID(), 'awl_em_settings_' . get_the_ID(), true);
                $em_id = $event_id;
                $em_start_date = $event_monster_output_settings['em_start_date'];
        ...
        $all_events_sorted_id_by_date = array();
        foreach ($all_event_dates as $key => $values) {
                        ...
                        <div class="module-inner">
                            <div class="module-content collapse in" id="content-4">
                                <div class="module-content-inner no-padding-bottom">
                                    <div class="panel-group-theme-1" id="accordion-4" role="tablist"
                                        ...
                                        while ($loop->have_posts()) :
                                            ...
                                            <figure class="panel panel-default event-pannel-1">
                                                <div class="panel-heading panel-heading-theme-1 icon-right" role="tab"
                                                     id="headingOne-<?php echo esc_attr($count); ?>"
                                                     data-toggle="collapse" data-parent="#accordion-4"
                                                     href="#collapseOne-<?php echo esc_attr($count); ?>"
                                                     aria-expanded="true"
                                                       ...
                                                       aria-controls="collapseOne-<?php echo esc_attr($count); ?>">
                                                        ...
                                                        <ul class="event-list">
                                                            <li onclick="return SingleEvent<?php echo esc_attr($em_id); ?>('singleeventshow','<?php echo esc_attr($em_id); ?>');">
                                                                ...
                                                                // Assuming $em_upload_images is an array of image URLs
                                                                if ($em_upload_image) {
                                                                    // Get image URLs for all images in the loop
                                                                    $image_urls_array = array($em_upload_image);

                                                                    // Get image URLs for the single image in the loop
                                                                    $image_urls = pippin_get_image_urls($em_upload_image);

                                                                    ?>
                                                                    <img src="<?php echo esc_url($image_urls['medium']); ?>"/>
                                                                    <?php
                                                ...
                                                <div id="collapseOne-<?php echo esc_attr($count); ?>"
                                                    ...
                                                    <?php } else { ?>
                                                        <div class="panel-body" id="event-show-<?php echo esc_attr($em_id); ?>">
                                                            <div id="single_load_<?php echo esc_attr($em_id); ?>"
                                                                 class="loader-wrapper loader-wrapper-4"
